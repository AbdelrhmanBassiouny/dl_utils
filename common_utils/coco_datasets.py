...
class CocoDataset():
    ...
    def get_labels(self, width=None, height=None,
                   out_format=('x1', 'y1', 'w', 'h'), relative=False, sort=True):
        all_imgs_labels = {}
        ...
        for img_id in self.segmentations.keys():
            ...
            for segm in self.segmentations[img_id]:
                ...
                bbox[1] *= height_ratio
                bbox[3] *= height_ratio
                new_bbox = convert_format(bbox, in_format=self.format,
        ...
        if sort:
            ...
            self.all_imgs_labels = deepcopy(all_imgs_labels)
        self.format = out_format
        self.isrel = True if relative else False
            ...
            for c in img:
                for box in img[c]:
                    ...
                    if out_format is not None:
                        bbox[:4] = convert_format(
                            bbox[:4], self.format, out_format)
    ...
    def read_labels_from_files(self, directory, in_format=None, out_format=('x1', 'y1', 'w', 'h'),
                               isrel=False, has_conf=True, add_conf=False, box_formatter=lambda x: float(x), sort=True):
    ...
    def get_labels_as(self, out_format=('x1', 'y1', 'w', 'h'), relative=False,
                      dtype='float', width=1.0, height=1.0, clsids=False, imgids=False):
        all_imgs_labels = {}
        img_id = 0
        for img_name, annotations in self.all_imgs_labels.items():
            img = img_name
            if imgids:
              img = img_id
            all_imgs_labels[img] = {}
            ...
            if relative and not self.isrel:
                ...
                width_ratio *= width
                height_ratio *= height
            for cls_name, boxes in annotations.items():
              id = self.cname2cid[cls_name]
              all_imgs_labels[img][id] = []
              for box in boxes:
                new_box = []
                ...
                new_box[0:4] = convert_format(new_box[0:4], in_format=self.format,
                                              out_format=out_format)
                if dtype == 'int':
